...
Future<Movie> getMovie({int year = 0, required int difficulty}) async {
  ...
  // the list is sorted by most popular, which gives us a different sort of difficulty setting.
  // the user can widen the pool of available answers by upping the diff setting
  final int pageNumber = (math.Random().nextInt(difficulty*3) + 1);

  // and if no year is given, we give ourselves a random one
  if (year == 0) {
    year = math.Random().nextInt(40) + 1980;
  }
